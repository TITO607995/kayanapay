...
export default function MasukPage() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      const res = await fetch("http://192.168.1.7:8000/api/member/login", {
        method: "POST",
      ...
      if (data.status === "success") {
        ...
        // Jeda 1.5 detik biar asik, baru pindah ke dashboard
        setTimeout(() => {
          router.push("/member");
        }, 1500);
      } else {
        ...
        <div className="bg-white rounded-3xl shadow-lg border border-slate-100 p-8">
          ...
          <form onSubmit={handleLogin} className="space-y-5">
            ...
            <div>
              ...
              <input type="text" name="username" required value={formData.username} onChange={handleChange} placeholder="Masukkan username" className="w-full rounded-xl border border-slate-200 px-4 py-3 outline-none focus:border-blue-500 focus:ring-4 focus:ring-blue-100 lowercase" />
            </div>
            ...
            <div>
              ...
              <div className="relative">
                <input type={showPassword ? "text" : "password"} name="password" required value={formData.password} onChange={handleChange} placeholder="Masukkan password" className="w-full rounded-xl border border-slate-200 px-4 py-3 outline-none focus:border-blue-500 focus:ring-4 focus:ring-blue-100" />
            ...
            <div className="text-right">
              <button type="button" onClick={() => alert("Fitur lupa password segera hadir!")} className="text-sm text-blue-600 hover:text-blue-700 font-medium">
                Lupa Password?
              </button>
            </div>
          ...
          <div className="mt-6 text-center text-sm text-slate-500">
            Belum punya akun?{" "}
            <Link href="/daftar" className="text-blue-600 font-semibold hover:underline">Daftar</Link>
          </div>
        </div>
      </div>
    </main>
