...
export default function FreeFire() {
  ...
  useEffect(() => {
    fetch("http://192.168.100.17:8000/api/topup/products?brand=FREE FIRE")
      .then((res) => res.json())
      .then((data) => {
  ...
  // FITUR BARU: AUTO-DETECT NICKNAME FREE FIRE
  useEffect(() => {
    if (userId.length >= 5) { // ID FF biasanya panjang
      setIsChecking(true);
      setNickname("");

      const timer = setTimeout(async () => {
        try {
          const res = await fetch(`http://192.168.100.17:8000/api/topup/check-ff?userid=${userId}`);
  ...
  const handleCheckout = async () => {
    if (!userId || !whatsapp) {
      alert("Harap lengkapi Player ID dan Nomor WhatsApp!");
      return;
    }
    ...
    try {
      const response = await fetch("http://192.168.100.17:8000/api/payment/checkout", {
        ...
        body: JSON.stringify({
          target: `${userId}`, // FF Cuma butuh Player ID (tanpa Zone ID)
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className={`space-y-6 ${activeTab === "transaksi" ? "block animate-fade-in" : "hidden lg:block"}`}>
              ...
              {/* STEP 1: Akun (Khusus FF cuma 1 Input) */}
              ...
              <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
                ...
                {isLoading ? (
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                    {products
                      .filter(item => !item.product_name.toLowerCase().includes('cek')) // Filter item 'cek' kalau ada
            ...
            {/* KONTEN KETERANGAN (Hanya muncul di Mobile saat tab Keterangan aktif, di Desktop disembunyikan pakai lg:hidden) */}
            <div className={`${activeTab === "keterangan" ? "block animate-fade-in" : "hidden"} lg:hidden bg-white p-6 rounded-2xl shadow-sm border border-slate-200`}>
              ...
              <p className="text-slate-600 text-sm leading-relaxed mb-6">
                Top up diamond Free Fire (FF) harga paling murah, aman, cepat, dan terpercaya hanya di KAYANAPAY. Beli diamond FF untuk beli Booyah Pass, gacha skin senjata, dan bundle impianmu!
              </p>
              ...
              <ol className="list-decimal ml-4 space-y-2 text-slate-600 text-sm font-medium">
                ...
                <li>Masukkan Data Akun (Player ID).</li>
