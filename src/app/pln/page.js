              ...
              <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
                ...
                <div className="w-full md:w-2/3">
                  <input 
                    ...
                    className="w-full bg-slate-50 border border-slate-300 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-blue-500 transition font-bold tracking-widest" 
                  />
                </div>
                
                {/* Tampilan Cek Nama PLN Otomatis */}
                <div className="h-6 mt-2">
                  {isChecking && <p className="text-xs text-blue-500 font-bold animate-pulse">⏳ Mengecek data pelanggan PLN...</p>}
                  {!isChecking && customerName && (
                    <p className={`text-sm font-bold tracking-wide ${customerName.includes("❌") ? "text-red-500" : "text-blue-600"}`}>
                      {customerName}
                    </p>
                  )}
                </div>
            ...
            <div className="space-y-6 h-full">
              ...
              <div className="sticky top-24 bg-white rounded-2xl p-6 border border-slate-200 shadow-[0_8px_30px_rgb(0,0,0,0.04)]">
                ...
                <div className="flex justify-between items-center mb-6 pt-2">
                  <span className="text-slate-800 font-bold">Total Bayar</span><span className="text-blue-600 font-black text-xl">{formatRupiah(finalTotal)}</span>
                </div>

                <button 
                  onClick={handleCheckout} 
                  disabled={!selectedDenom || !selectedPayment || !meterNumber || !whatsapp || isCheckoutLoading} 
                  className={`w-full font-black py-4 rounded-xl shadow-lg transition-all flex items-center justify-center gap-2 ${selectedDenom && selectedPayment && meterNumber && whatsapp && !isCheckoutLoading ? "bg-blue-600 hover:bg-blue-700 text-white shadow-blue-600/30 cursor-pointer" : "bg-slate-200 text-slate-400 cursor-not-allowed shadow-none"}`}
                >
                  {isCheckoutLoading ? (
