...
function TokenPln() {
  ...
  const [activeTab, setActiveTab] = useState("transaksi"); // Untuk UI Mobile
  ...
  useEffect(() => {
    ...
    fetch("https://kayanamart.my.id/api/topup/products?brand=PLN")
      .then((res) => res.json())
      .then((data) => {
  ...
  // FITUR: AUTO-DETECT NAMA PELANGGAN PLN
  useEffect(() => {
    if (meterNumber.length >= 11) { // Nomor meter PLN biasanya minimal 11 digit
      setIsChecking(true);
      setCustomerName("");
      ...
      const timer = setTimeout(async () => {
        try {
          ...
          const res = await fetch(`https://kayanamart.my.id/api/topup/check-pln?meter=${meterNumber}`);
          const data = await res.json();

          if (data.status === "success") {
            setCustomerName(`✅ A/N: ${data.data.nickname}`); 
          } else {
            setCustomerName("❌ Nomor Meter tidak ditemukan");
          }
        } catch (error) {
          setCustomerName("❌ Gagal mengecek server PLN");
        } finally {
          setIsChecking(false);
        }
      }, 1500); // Kasih jeda 1.5 detik biar ga spam API saat user ngetik
  ...
  const handleCheckout = async () => {
    if (!meterNumber || !whatsapp || !selectedDenom) {
      alert("Harap lengkapi No. Meteran, Nominal, dan Nomor WhatsApp!");
      return;
    }
    ...
    <main className="min-h-screen font-sans pb-36 lg:pb-20 bg-slate-50 relative">
      <div className="container mx-auto px-4 py-8 max-w-6xl">
        ...
        <div className="flex items-center gap-4 mb-8 bg-white p-5 rounded-2xl shadow-sm border border-slate-200">
          ...
          <div>
            <h2 className="text-2xl font-black text-slate-800 uppercase">Token PLN</h2>
            <p className="text-sm text-slate-500">Arus Listrik Menyala Otomatis 24 Jam.</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            {/* KONTEN TRANSAKSI (Di Desktop selalu nampil, di Mobile ikut state) */}
            <div className={`space-y-6 ${activeTab === "transaksi" ? "block animate-fade-in" : "hidden lg:block"}`}>
              ...
              <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
                ...
                <div className="w-full md:w-2/3">
                  <input 
                    type="number" 
                    placeholder="Masukkan 11-12 digit No. Meter" 
                    value={meterNumber} 
                    onChange={(e) => setMeterNumber(e.target.value)} 
                    className="w-full bg-slate-50 border border-slate-300 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-emerald-500 transition font-bold tracking-widest" 
                  />
                </div>
                
                {/* Tampilan Cek Nama PLN Otomatis */}
                <div className="h-6 mt-2">
                  {isChecking && <p className="text-xs text-emerald-500 font-bold animate-pulse">⏳ Mengecek data pelanggan PLN...</p>}
                  {!isChecking && customerName && (
                    <p className={`text-sm font-bold tracking-wide ${customerName.includes("❌") ? "text-red-500" : "text-emerald-600"}`}>
                      {customerName}
                    </p>
                  )}
                </div>
              ...
              <div id="step-nominal" className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
                ...
                {isLoading ? (
                  <div className="text-center py-10"><p className="text-slate-500 animate-pulse font-semibold">Mengambil nominal Token PLN...</p></div>
                ) : (
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                    {products
                      .filter(item => !item.product_name.toLowerCase().includes('cek')) // Sembunyiin produk "Cek Nama" yang jelek dari layar user
                      .sort((a, b) => a.price_sell - b.price_sell).map((item) => (
                      <button 
                      ...
                      >
                        <p className="font-bold text-sm text-slate-800">{item.product_name.replace("PLN PREPAID - ", "")}</p>
                        <p className="text-xs font-black text-emerald-600 mt-2">{formatRupiah(item.price_sell)}</p>
                      </button>
                    ))}
                  </div>
                )}
            ...
            <div className={`${activeTab === "keterangan" ? "block animate-fade-in" : "hidden"} lg:hidden bg-white p-6 rounded-2xl shadow-sm border border-slate-200`}>
              ...
              <p className="text-slate-600 text-sm leading-relaxed mb-6">
                Beli Token Listrik PLN Prabayar dengan harga termurah, cepat, dan otomatis masuk dalam hitungan detik. Layanan aktif 24 jam nonstop setiap hari!
              </p>
              
              <h4 className="text-slate-800 font-bold text-sm mb-3">Cara Membeli Token PLN :</h4>
              <ol className="list-decimal ml-4 space-y-2 text-slate-600 text-sm font-medium">
                <li>Pilih tab <b>Transaksi</b>.</li>
                <li>Masukkan <b>Nomor Meter / ID Pelanggan</b> PLN Anda.</li>
                <li>Sistem akan otomatis mengecek nama pemilik meteran.</li>
                <li>Pilih Nominal Token yang diinginkan.</li>
                <li>Pilih Metode Pembayaran.</li>
                <li>Masukkan Nomor <b>WhatsApp</b> yang aktif.</li>
                <li>Klik <b>Pesan Sekarang</b> dan lakukan Pembayaran.</li>
                <li>Token (Nomor Seri) akan langsung dikirim ke WhatsApp Anda!</li>
              </ol>
            </div>
          ...
          {/* ========================================== */}
          {/* KOLOM KANAN (SUMMARY DESKTOP ONLY)           */}
          {/* ========================================== */}
          <div className="hidden lg:block lg:col-span-1 relative">
            <div className="space-y-6 h-full">
              ...
              <div className="bg-white rounded-2xl p-6 border border-slate-200 shadow-sm">
                ...
                <h4 className="text-slate-800 font-bold text-sm mb-3">Cara Membeli Token PLN :</h4>
                <ol className="list-decimal ml-4 space-y-2 text-slate-600 text-sm font-medium">
                  <li>Masukkan <b>Nomor Meter / ID Pelanggan</b>.</li>
                  <li>Sistem akan otomatis mengecek nama Anda.</li>
                  <li>Pilih Nominal Token yang diinginkan.</li>
                  <li>Pilih Metode Pembayaran.</li>
                  <li>Masukkan Nomor <b>WhatsApp</b> yang aktif.</li>
                  <li>Klik <b>Bayar Sekarang</b> dan selesaikan Pembayaran.</li>
                  <li>Token SN akan dikirim ke WhatsApp Anda.</li>
                </ol>
              </div>

              <div className="sticky top-24 bg-white rounded-2xl p-6 border border-slate-200 shadow-[0_8px_30px_rgb(0,0,0,0.04)]">
                <h3 className="font-black text-lg text-slate-800 border-b border-slate-100 pb-4 mb-4">Rincian Pembelian</h3>
                ...
                <div className="flex items-center gap-4 mb-6">
                  ...
                  <div>
                    <h4 className="text-slate-800 font-bold text-sm">Token PLN</h4>
                    <p className="text-slate-500 text-xs mt-0.5">
                      {selectedDenom ? selectedDenom.product_name.replace("PLN PREPAID - ", "") : "Belum memilih Token"}
                    </p>
                  </div>
                </div>
                ...
                <div className="space-y-3 mb-6 bg-slate-50 p-4 rounded-xl border border-slate-100">
                  ...
                  {useKoin && koinDigunakan > 0 && (
                    <div className="flex justify-between text-sm">
                      <span className="text-slate-500 font-medium">Koin ({koinDigunakan} koin)</span>
                      <span className="text-purple-600 font-bold">-{formatRupiah(koinDiskon)}</span>
                    </div>
                  )}
                </div>

                <div className="flex justify-between items-center mb-6 pt-2">
                  <span className="text-slate-800 font-bold">Total Bayar</span><span className="text-emerald-600 font-black text-xl">{formatRupiah(finalTotal)}</span>
                </div>

                <button 
                  onClick={handleCheckout} 
                  disabled={!selectedDenom || !selectedPayment || !meterNumber || !whatsapp || isCheckoutLoading} 
                  className={`w-full font-black py-4 rounded-xl shadow-lg transition-all flex items-center justify-center gap-2 ${selectedDenom && selectedPayment && meterNumber && whatsapp && !isCheckoutLoading ? "bg-emerald-600 hover:bg-blue-700 text-white shadow-emerald-600/30 cursor-pointer" : "bg-slate-200 text-slate-400 cursor-not-allowed shadow-none"}`}
                >
                  {isCheckoutLoading ? (
      ...
      <div className="fixed bottom-0 left-0 w-full bg-white border-t border-slate-200 p-4 z-50 lg:hidden rounded-t-[2rem] shadow-[0_-10px_30px_rgba(0,0,0,0.06)] flex flex-col gap-3 transition-transform">
        ...
        <button
          ...
          className={`w-full font-black py-4 rounded-xl transition-all flex items-center justify-center gap-2 uppercase tracking-wide text-sm ${
            selectedDenom && selectedPayment && meterNumber && whatsapp && !isCheckoutLoading
...
export default function TokenPlnContent() {
  return (
