...
export default function TokenPln() {
  ...
  useEffect(() => {
    ...
    fetch("http://192.168.100.17:8000/api/topup/products?brand=PLN")
      .then((res) => res.json())
      .then((data) => {
    ...
    if (meterNumber.length >= 11) { // Nomor meter PLN biasanya minimal 11 digit
      ...
      const timer = setTimeout(async () => {
        try {
          ...
          const res = await fetch(`http://192.168.100.17:8000/api/topup/check-pln?meter=${meterNumber}`);
  ...
  const handleCheckout = async () => {
    ...
    try {
      const response = await fetch("http://192.168.100.17:8000/api/payment/checkout", {
        ...
        body: JSON.stringify({
          target: meterNumber, // Target cuma pakai nomor meteran aja
