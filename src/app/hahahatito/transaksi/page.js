...
export default function KelolaTransaksiPage() {
  ...
  const fetchTransactions = async () => {
    setIsLoading(true);
    try {
      const res = await fetch("http://192.168.100.17:8000/api/admin/transactions");
  ...
  const handleForceSuccess = async (reference) => {
    ...
    try {
      const res = await fetch("http://192.168.100.17:8000/api/admin/transactions/force-success", {
        method: "POST",
  ...
  const handleRetry = async (reference) => {
    ...
    try {
      const res = await fetch("http://192.168.100.17:8000/api/admin/transactions/retry", {
        method: "POST",
        headers: { "Content-Type": "application/json", "Accept": "application/json" },
        body: JSON.stringify({ reference })
      });
