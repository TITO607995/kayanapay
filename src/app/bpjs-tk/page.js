...
export default function BpjsTK() {
  ...
  useEffect(() => {
    fetch("https://kayanamart.my.id/api/payment/methods").then(r => r.json()).then(d => { if (d.status === "success") setPaymentMethods(d.data); });
  }, []);
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      if (resData.status === "success") window.location.href = resData.data.paymentUrl; 
        ...
        setErrorMessage("Gagal: " + resData.message);
        window.scrollTo({ top: 0, behavior: 'smooth' });
      }
    } catch { 
