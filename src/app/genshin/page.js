...
export default function GenshinImpact() {
  ...
  const [zoneId, setZoneId] = useState(""); // Dipakai untuk Server Genshin
  ...
  useEffect(() => {
    fetch("http://192.168.100.17:8000/api/topup/products?brand=GENSHIN IMPACT")
      .then((res) => res.json())
      .then((data) => {
  ...
  // FITUR: AUTO-DETECT NICKNAME GENSHIN
  useEffect(() => {
    if (userId.length >= 8 && zoneId !== "") { // UID Genshin biasanya 9 angka
      setIsChecking(true);
      setNickname("");
      ...
      const timer = setTimeout(async () => {
        try {
          ...
          const res = await fetch(`http://192.168.100.17:8000/api/topup/check-genshin?userid=${userId}&zoneid=${zoneId}`);
  ...
  const handleCheckout = async () => {
    if (!userId || !zoneId || !whatsapp) {
      alert("Harap lengkapi UID, Server, dan Nomor WhatsApp!");
      return;
    }
    ...
    try {
      const response = await fetch("http://192.168.100.17:8000/api/payment/checkout", {
        ...
        body: JSON.stringify({
          target: `${userId}${zoneId}`, // UID + Server code (e.g. 812345678os_asia)
    ...
    <main className="min-h-screen font-sans pb-36 lg:pb-20 bg-slate-50 relative">
      <div className="container mx-auto px-4 py-8 max-w-6xl">
        ...
        <div className="flex items-center gap-4 mb-8 bg-white p-5 rounded-2xl shadow-sm border border-slate-200">
          ...
          <div>
            <h2 className="text-2xl font-black text-slate-800 uppercase">Genshin Impact</h2>
            <p className="text-sm text-slate-500">Hoyoverse | Proses Otomatis 1 Detik.</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className={`space-y-6 ${activeTab === "transaksi" ? "block animate-fade-in" : "hidden lg:block"}`}>
              ...
              {/* STEP 1: Akun (UID + Server Dropdown) */}
              <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
                ...
                <div className="flex flex-col gap-1">
                  ...
                  <p className="text-xs text-slate-400 mt-2">Masukkan UID dan pilih Server yang sesuai dengan akun Anda.</p>
                </div>
              </div>
              ...
              <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
                ...
                {isLoading ? (
                  <div className="text-center py-10">
                    <p className="text-slate-500 animate-pulse font-semibold">Mengambil daftar harga...</p>
                  </div>
                ) : (
                  <div className="space-y-8">
                    {/* KATEGORI 1: WELKIN MOON */}
                    {products.filter(p => p.product_name.toLowerCase().includes('blessing') || p.product_name.toLowerCase().includes('welkin')).length > 0 && (
                      <div>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                          {products
                            .filter(item => item.product_name.toLowerCase().includes('blessing') || item.product_name.toLowerCase().includes('welkin'))
                            ...
                            .map((item) => (
                              ...
                              >
                                <p className="font-bold text-sm text-slate-800">{item.product_name.replace("GENSHIN IMPACT - ", "")}</p>
                    ...
                    <div>
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                        {products
                          .filter(item => !item.product_name.toLowerCase().includes('blessing') && !item.product_name.toLowerCase().includes('welkin') && !item.product_name.toLowerCase().includes('cek'))
                          .sort((a, b) => a.price_sell - b.price_sell)
                          .map((item) => (
                            <button 
                              key={item.buyer_sku_code}
            ...
            <div className={`${activeTab === "keterangan" ? "block animate-fade-in" : "hidden"} lg:hidden bg-white p-6 rounded-2xl shadow-sm border border-slate-200`}>
              ...
              <p className="text-slate-600 text-sm leading-relaxed mb-6">
                Top up Genesis Crystal dan Blessing of the Welkin Moon harga paling murah, aman, cepat, dan terpercaya hanya di KAYANAPAY. Dapatkan primogems untuk gacha karakter bintang 5 incaranmu!
              </p>
              ...
              <ol className="list-decimal ml-4 space-y-2 text-slate-600 text-sm font-medium">
                ...
                <li>Masukkan <b>UID</b> dan pilih <b>Server</b> Anda.</li>
                <li>Pilih nominal Genesis Crystal atau Welkin Moon.</li>
            ...
            <div className="space-y-6 h-full">
              ...
              <div className="sticky top-24 bg-white rounded-2xl p-6 border border-slate-200 shadow-[0_8px_30px_rgb(0,0,0,0.04)]">
                ...
                <div className="flex items-center gap-4 mb-6">
                  ...
                  <div>
                    <h4 className="text-slate-800 font-bold text-sm">Genshin Impact</h4>
                    <p className="text-slate-500 text-xs mt-0.5">
                      {selectedDenom ? selectedDenom.product_name.replace("GENSHIN IMPACT - ", "") : "Belum memilih nominal"}
                    </p>
                  </div>
                </div>
