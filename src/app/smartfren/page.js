...
export default function Smartfren() {
  ...
  useEffect(() => {
    fetch("http://192.168.100.17:8000/api/topup/products?brand=SMARTFREN")
      .then((res) => res.json())
      .then((data) => {
  ...
  const handleCheckout = async () => {
    if (!phoneNumber || !whatsapp || !selectedDenom) {
      alert("Harap lengkapi Nomor HP dan Nomor WhatsApp!");
      return;
    }
    ...
    <main className="min-h-screen font-sans pb-36 lg:pb-20 bg-slate-50 relative">
      <div className="container mx-auto px-4 py-8 max-w-6xl">
        ...
        <div className="flex items-center gap-4 mb-8 bg-white p-5 rounded-2xl shadow-sm border border-slate-200">
          ...
          <div>
            <h2 className="text-2xl font-black text-slate-800 uppercase">Smartfren</h2>
            <p className="text-sm text-slate-500">Pulsa & Paket Data | Proses Otomatis.</p>
          </div>
        </div>
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className={`space-y-6 ${activeTab === "transaksi" ? "block animate-fade-in" : "hidden lg:block"}`}>
              ...
              <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
                ...
                <div className="w-full md:w-2/3">
                  <input 
                    ...
                    className="w-full bg-slate-50 border border-slate-300 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-blue-500 transition font-bold tracking-wider" 
                  />
                </div>
              </div>
              ...
              <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
                ...
                {isLoading ? (
                  ...
                  <div className="space-y-8">
                    ...
                    {products.filter(p => p.category === "Pulsa" || p.product_name.toLowerCase().includes("pulsa")).length > 0 && (
                      <div>
                        <h4 className="text-sm font-bold text-slate-700 mb-4 border-b border-slate-100 pb-2 flex items-center gap-2">
                          📞 Pulsa Reguler
                        </h4>
                        <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                          {products
                            .filter(item => item.category === "Pulsa" || item.product_name.toLowerCase().includes("pulsa"))
                            ...
                            .map((item) => (
                              ...
                              >
                                <p className="font-bold text-sm text-slate-800">{item.product_name.replace("SMARTFREN ", "")}</p>
                                <p className="text-xs font-black text-blue-600 mt-2">{formatRupiah(item.price_sell)}</p>
                              </button>
                            ))}
                        </div>
                      </div>
                    ...
                    {products.filter(p => p.category === "Data" || p.product_name.toLowerCase().includes("data") || p.product_name.toLowerCase().includes("combo") || p.product_name.toLowerCase().includes("internet")).length > 0 && (
                      <div>
                        <h4 className="text-sm font-bold text-slate-700 mb-4 border-b border-slate-100 pb-2 flex items-center gap-2">
                          🌐 Paket Data & Internet
                        </h4>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                          {products
                            .filter(item => item.category === "Data" || item.product_name.toLowerCase().includes("data") || item.product_name.toLowerCase().includes("combo") || item.product_name.toLowerCase().includes("internet"))
                            .sort((a, b) => a.price_sell - b.price_sell)
                            .map((item) => (
                              <button 
                                key={item.buyer_sku_code} 
              ...
              <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
                ...
                {paymentMethods.length === 0 && !isLoading ? ( <p className="text-sm text-red-500 font-medium">Metode pembayaran sedang tidak tersedia.</p> ) : (
                          ...
                          return (
                            ...
                            >
                              <div className="flex justify-between items-center w-full mb-3">
                                <div className="h-8"><img src={displayImage} alt="QRIS" className="h-full object-contain" /></div>
                              ...
                              <div className="flex justify-between items-end w-full">
                                <p className="font-bold text-sm text-slate-800 text-left">QRIS All Payment</p>
                                <p className="text-xs font-black text-blue-600 bg-blue-100 px-2 py-1 rounded-md">{pay.totalFee == 0 ? "Gratis" : `+ ${formatRupiah(pay.totalFee)}`}</p>
                              </div>
                            </button>
                          );
                        })}
                      ...
                      {categoryOrder.filter(cat => cat !== "QRIS").map((category) => {
                        ...
                        return (
                          <div key={category} className={`border rounded-xl overflow-hidden transition-all duration-300 ${isOpen ? 'border-blue-300 shadow-sm' : 'border-slate-200'}`}>
                            <button onClick={() => setOpenCategory(isOpen ? null : category)} className={`w-full flex items-center justify-between p-4 transition-colors ${isOpen ? 'bg-blue-50' : 'bg-white hover:bg-slate-50'}`}>
                              <div className="flex items-center gap-3"><span className="text-xl">{categoryIcons[category]}</span><span className={`font-bold ${isOpen ? 'text-blue-600' : 'text-slate-700'}`}>{category}</span></div>
                              <svg xmlns="http://www.w3.org/2000/svg" className={`h-5 w-5 transition-transform duration-300 ${isOpen ? 'rotate-180 text-blue-500' : 'text-slate-400'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" /></svg>
                            </button>
                            {isOpen && (
                              <div className="p-4 bg-slate-50 grid grid-cols-1 md:grid-cols-2 gap-3 border-t border-blue-100">
                                {methods.map((pay) => (
                                  <button key={pay.paymentMethod} onClick={() => setSelectedPayment(pay)} className={`w-full p-3 rounded-xl border flex flex-col justify-center transition-all ${selectedPayment?.paymentMethod === pay.paymentMethod ? "border-blue-500 bg-white ring-1 ring-blue-500 shadow-md" : "border-slate-200 bg-white hover:border-blue-300 hover:shadow-sm"}`}>
                                    <div className="flex justify-between items-center w-full mb-2">
                                      <div className="h-6"><img src={pay.paymentImage} alt={pay.paymentName} className="h-full object-contain" /></div>
                                      <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center ${selectedPayment?.paymentMethod === pay.paymentMethod ? 'border-blue-500' : 'border-slate-300'}`}>{selectedPayment?.paymentMethod === pay.paymentMethod && <div className="w-2 h-2 bg-blue-500 rounded-full"></div>}</div>
                                    </div>
                                    <div className="flex justify-between items-end w-full mt-1">
                                      <p className="font-bold text-xs text-slate-700 text-left line-clamp-1">{pay.paymentName}</p>
                                      <p className="text-[11px] font-black text-blue-500 whitespace-nowrap">{pay.totalFee == 0 ? "Gratis" : `+ ${formatRupiah(pay.totalFee)}`}</p>
                                    </div>
                                  </button>
                                ))}
                              </div>
            ...
            <div className={`${activeTab === "keterangan" ? "block animate-fade-in" : "hidden"} lg:hidden bg-white p-6 rounded-2xl shadow-sm border border-slate-200`}>
              ...
              <p className="text-slate-600 text-sm leading-relaxed mb-6">
                Isi ulang Pulsa Reguler dan Paket Data Smartfren dengan harga terjangkau. Transaksi diproses secara otomatis dalam hitungan detik!
              </p>
              
              <h4 className="text-slate-800 font-bold text-sm mb-3">Cara Mengisi Pulsa :</h4>
              <ol className="list-decimal ml-4 space-y-2 text-slate-600 text-sm font-medium">
                <li>Pilih tab <b>Transaksi</b>.</li>
                <li>Masukkan <b>Nomor HP</b> Anda.</li>
                <li>Pilih Nominal Pulsa atau Paket Data.</li>
                <li>Tentukan Jumlah Pembelian.</li>
                <li>Pilih Metode Pembayaran.</li>
                <li>Masukkan Nomor <b>WhatsApp</b> yang aktif.</li>
            ...
            <div className="space-y-6 h-full">
              ...
              <div className="sticky top-24 bg-white rounded-2xl p-6 border border-slate-200 shadow-[0_8px_30px_rgb(0,0,0,0.04)]">
                ...
                <div className="flex items-center gap-4 mb-6">
                  ...
                  <div>
                    <h4 className="text-slate-800 font-bold text-sm">Smartfren</h4>
                    <p className="text-slate-500 text-xs mt-0.5">
                      {selectedDenom ? selectedDenom.product_name.replace("SMARTFREN ", "") : "Belum memilih produk"}
                    </p>
                  </div>
                </div>
                
                <div className="space-y-3 mb-6 bg-slate-50 p-4 rounded-xl border border-slate-100">
                  <div className="flex justify-between text-sm"><span className="text-slate-500">Harga</span><span className="text-slate-800 font-bold">{formatRupiah(price)}</span></div>
                  <div className="flex justify-between text-sm"><span className="text-slate-500">Jumlah</span><span className="text-slate-800 font-bold">{qty}x</span></div>
                  <div className="flex justify-between text-sm"><span className="text-slate-500">Admin</span><span className="text-slate-800 font-bold">{formatRupiah(adminFee)}</span></div>
                </div>

                <div className="flex justify-between items-center mb-6 pt-2">
                  <span className="text-slate-800 font-bold">Total Bayar</span><span className="text-blue-600 font-black text-xl">{formatRupiah(grandTotal)}</span>
                </div>

                <button 
                  onClick={handleCheckout} 
                  disabled={!selectedDenom || !selectedPayment || !phoneNumber || !whatsapp || isCheckoutLoading} 
                  className={`w-full font-black py-4 rounded-xl shadow-lg transition-all flex items-center justify-center gap-2 ${
                    selectedDenom && selectedPayment && phoneNumber && whatsapp && !isCheckoutLoading 
