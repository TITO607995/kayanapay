...
function Telkomsel() {
  ...
  useEffect(() => {
    fetch("https://kayanamart.my.id/api/topup/products?brand=TELKOMSEL")
      .then((res) => res.json())
      .then((data) => {
              ...
              <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
                ...
                <div className="w-full md:w-2/3">
                  <input 
                    ...
                    className="w-full bg-slate-50 border border-slate-300 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-emerald-500 transition font-bold tracking-wider" 
                  />
                </div>
              </div>
                            ...
                            .map((item) => (
                              <button 
                                ...
                                className={`p-4 rounded-xl border text-left transition-all flex flex-col justify-between h-full ${
                                  ...
                                  : "border-slate-200 bg-slate-50 hover:border-emerald-300 hover:shadow-sm"
                                }`}
                              >
                          ...
                          return (
                            ...
                            >
                              ...
                              <div className="flex justify-between items-end w-full">
                                ...
                                <p className="text-xs font-black text-emerald-600 bg-emerald-100 px-2 py-1 rounded-md">{pay.totalFee == 0 ? "Gratis" : `+ ${formatRupiah(pay.totalFee)}`}</p>
                              </div>
                            </button>
                          );
                        })}
                                ...
                                {methods.map((pay) => (
                                  <button key={pay.paymentMethod} onClick={() => setSelectedPayment(pay)} className={`w-full p-3 rounded-xl border flex flex-col justify-center transition-all ${selectedPayment?.paymentMethod === pay.paymentMethod ? "border-emerald-500 bg-white ring-1 ring-emerald-500 shadow-md" : "border-slate-200 bg-white hover:border-emerald-300 hover:shadow-sm"}`}>
                                    <div className="flex justify-between items-center w-full mb-2">
                                      ...
                                      <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center ${selectedPayment?.paymentMethod === pay.paymentMethod ? 'border-emerald-500' : 'border-slate-300'}`}>{selectedPayment?.paymentMethod === pay.paymentMethod && <div className="w-2 h-2 bg-emerald-500 rounded-full"></div>}</div>
                                    </div>
                                    <div className="flex justify-between items-end w-full mt-1">
                                      <p className="font-bold text-xs text-slate-700 text-left line-clamp-1">{pay.paymentName}</p>
                                      <p className="text-[11px] font-black text-emerald-500 whitespace-nowrap">{pay.totalFee == 0 ? "Gratis" : `+ ${formatRupiah(pay.totalFee)}`}</p>
                                    </div>
                                  </button>
                                ))}
                              </div>
...
export default function TelkomselContent() {
  return (
